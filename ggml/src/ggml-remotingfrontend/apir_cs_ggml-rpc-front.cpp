...
apir_rpc_tensor
apir_serialize_tensor(const ggml_tensor * tensor) {
...
void
apir_add_tensor(ggml_tensor * tensor, std::vector<apir_rpc_tensor> & tensors, std::unordered_set<ggml_tensor*> & visited) {
    if (tensor == nullptr) {
        return;
    }
...
void
apir_serialize_graph(const ggml_cgraph * cgraph, std::vector<uint8_t> & output) {
    ...
    int output_size = sizeof(uint32_t) + n_nodes * sizeof(uint64_t) + sizeof(uint32_t) + n_tensors * sizeof(apir_rpc_tensor);
    ...
    *out_ntensors = n_tensors;
    apir_rpc_tensor * out_tensors = (apir_rpc_tensor *)(output.data() + sizeof(n_nodes) + n_nodes * sizeof(uint64_t) + sizeof(uint32_t));
