...
static inline void
apir_encode_rcp_tensor(struct apir_encoder *enc, const rpc_tensor *rpc_tensor) {
  size_t rpc_tensor_size = sizeof(*rpc_tensor);
  apir_encode(enc, rpc_tensor_size, rpc_tensor, rpc_tensor_size);
}

static inline rpc_tensor *
apir_decode_rpc_tensor_inplace(struct apir_decoder *dec) {
  size_t rpc_tensor_size = sizeof(rpc_tensor);

  return (rpc_tensor *)(uintptr_t) apir_decoder_use_inplace(dec, rpc_tensor_size);
}

static inline rpc_tensor *
apir_decode_rpc_tensor_array_inplace(struct apir_decoder *dec, uint32_t n_tensors) {
...
apir_decode_ggml_tensor(struct apir_decoder *dec) {
  const rpc_tensor *rpc_tensor = apir_decode_rpc_tensor_inplace(dec);
...
apir_decode_ggml_cgraph(struct apir_decoder *dec, size_t cgraph_size) {
  ...
  const rpc_tensor *tensors = apir_decode_rpc_tensor_array_inplace(dec, n_tensors);
