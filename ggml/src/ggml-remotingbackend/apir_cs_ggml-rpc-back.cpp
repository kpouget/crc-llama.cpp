...
void
apir_track_backend_buffer(ggml_backend_buffer_t buffer) {
    backend_buffers.insert(buffer);
}

bool
apir_untrack_backend_buffer(ggml_backend_buffer_t buffer) {
...
apir_get_track_backend_buffers() {
    return backend_buffers;
}

ggml_tensor *
apir_deserialize_tensor(struct ggml_context * ctx, const apir_rpc_tensor * tensor) {
...
ggml_cgraph *
apir_deserialize_graph(uint32_t n_nodes, uint32_t n_tensors, const apir_rpc_tensor * tensors, const uint64_t * nodes) {
